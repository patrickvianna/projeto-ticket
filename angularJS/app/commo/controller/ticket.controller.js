(function(){
angular.module('myApp').controller('TicketCtrl', ['$scope', '$http', '$location', 'consts', '$state', 'Msg', '$q', '$rootScope', '$uibModal', 'FileUploader', TicketController])

function TicketController($scope, $http, $location, consts, $state, Msg, $q, $rootScope,$uibModal, FileUploader){
    const vm = this
    ...
    function getProjetos() {
        return $q(function(resolve, reject) {
            ...
            .then(resp => {
                ...
                if(vm.projetos.lista.length < 1)
                {
                  Msg.addError('Não há projetos associados a esse usuário')
                  reject()
                }
                else
                  resolve()
